...
function safeNextPath(raw: string | null, fallback = "/collection") {
  ...
  // prevent open redirects: only allow site-relative paths
...
export default function LoginPage() {
  ...
  const next = useMemo(
    () => safeNextPath(searchParams.get("next"), "/collection"),
    [searchParams]
  );
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // one redirect target: ?next=... or default
      router.replace(next);
      router.refresh(); // ensures server components pick up authed state if/when you add cookies later
    } catch (err: any) {
