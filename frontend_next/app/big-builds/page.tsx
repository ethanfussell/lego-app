...
async function fetchBigBuilds(minPieces: number): Promise<{ sets: SetRow[]; total: number }> {
  const qs = new URLSearchParams();
  qs.set("page", "1");
  qs.set("limit", "60");
  qs.set("min_pieces", String(minPieces));
  qs.set("sort", "pieces");
  qs.set("order", "desc");
...
type Props = { searchParams: Promise<{ min?: string }> };

export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  const { min } = await searchParams;
  const minPieces = asFiniteNumber(min) ?? 500;
  const title = `LEGO Sets with ${minPieces.toLocaleString()}+ Pieces`;
  const description = `Browse large LEGO sets with ${minPieces.toLocaleString()} or more pieces, sorted by piece count.`;

  return {
    title,
    description,
    metadataBase: new URL(siteBase()),
    alternates: { canonical: "/big-builds" },
    openGraph: { title, description, url: "/big-builds", type: "website" },
...
export default async function Page({ searchParams }: Props) {
  const { min } = await searchParams;
  const minPieces = asFiniteNumber(min) ?? 500;
  const { sets, total } = await fetchBigBuilds(minPieces);
      ...
      <div className="pt-10">
        ...
        <div className="mt-3">
          ...
          <p className="mt-2 text-sm text-zinc-500">
            {total > 0 ? `${total.toLocaleString()} sets` : "Browse large LEGO builds."}
          </p>
        </div>
      </div>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 text-xs text-zinc-500">
                      ...
                      {typeof s.pieces === "number" && (
                        <><span className="mx-1">&bull;</span><span className="font-semibold text-zinc-900">{s.pieces.toLocaleString()} pcs</span></>
                      )}
                    ...
                    {typeof s.retail_price === "number" && s.retail_price > 0 && (
                      <div className="mt-1 text-xs text-zinc-500">${s.retail_price.toFixed(2)}</div>
                    )}
                  </div>
                </div>
              </div>
