// frontend_next/app/users/[username]/followers/page.tsx
...
const fetchFollowers = cache(async (username: string): Promise<{ total: number; users: FollowUser[] }> => {
  const url = new URL(`/api/users/${encodeURIComponent(username)}/followers?limit=100`, siteBase()).toString();
...
export async function generateMetadata({ params }: { params: Params | Promise<Params> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `People who follow @${username} on BrickTrack.`,
  };
}

export default async function FollowersPage({ params }: { params: Params | Promise<Params> }) {
  const p = await Promise.resolve(params);
  const username = normalizeUsername(p.username);
  if (!username) notFound();

  const { total, users } = await fetchFollowers(username);
    ...
    <div className="mx-auto w-full max-w-3xl px-6 pb-16">
      <div className="pt-10">
        ...
        <h1 className="mt-4 text-2xl font-bold text-zinc-900">
          Followers <span className="text-zinc-400">({total})</span>
        </h1>
      </div>

      {users.length === 0 ? (
        <p className="mt-6 text-sm text-zinc-500">No followers yet.</p>
      ) : (
