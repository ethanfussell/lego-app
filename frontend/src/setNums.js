// frontend/src/setNums.js

export function baseSetNum(setNum) {
    return String(setNum || "").trim().split("-")[0];
  }
  
  export function isFullSetNum(s) {
    const t = String(s || "").trim();
    const parts = t.split("-");
    return parts.length === 2 && parts[0].length > 0 && parts[1].length > 0;
  }
  
  export function toDeleteParam(setNum) {
    return baseSetNum(setNum);
  }
  
  export function listHasSetNum(listLike, setNum) {
