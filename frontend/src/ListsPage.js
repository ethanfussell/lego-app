  ...
  return (
    <Link
      ...
      style={{
        ...
        textDecoration: "none",
        color: "inherit",
        pointerEvents: "auto", // force-enable in case a parent did something weird
      }}
    >
...
function SortableListCard({ list, disabled }) {
  ...
  // IMPORTANT: only block clicks while actively dragging
  const innerStyle = { pointerEvents: isDragging ? "none" : "auto" };
...
export default function MyListsSection({ lists = [], onListsChange }) {
  ...
  async function persistOrder(nextItems, prevItems) {
    const orderedIds = nextItems.map((x) => Number(x.id));

    setSaving(true);
    try {
      // backend expects ordered_ids (per your lists.py)
