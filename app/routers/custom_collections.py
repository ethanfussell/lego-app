...
@router.post("/collection/owned", status_code=status.HTTP_201_CREATED, response_model=CollectionItem)
def add_owned(payload: CollectionCreate):
    ...
    OWNED.append(item)
    # ensure itr's not lingering in wishlist
...
def remove_owned(set_num: str, username: str = Query(..., description="Username to remove from owned")):
...
def add_wishlist(payload: CollectionCreate):
    ...
    # optional: if in owned, keep both or remove from owned; we'll keep both for now
    return item

@router.delete("/collections/wishlist/{set_num}", status_code=status.HTTP_204_NO_CONTENT)
def remove_wishlist(set_num: str, username: str = Query(..., description="Username to remove from owned")):
    if not _remove_item(OWNED, username, set_num):
        raise HTTPException(status_code=404, detail="Not in owned")

@router.post("/collections/wishlist", status_code=status.HTTP_201_CREATED, response_model=CollectionItem)
def add_wishlist(payload: CollectionCreate):
    if not _set_exists(payload.set.num):
        raise HTTPException(status_code=404, detail="Set not found")
    if _has_item(WISHLIST, payload.username, payload.set_num):
        raise HTTPException(status_code=400, detail="Already in wishlist")
    item = {
...
@router.delete("/collections/wishlist/{set_num}", status_code=status.HTTP_204_NO_CONTENT)
def remove_wishlist(set_num: str, username: str = Query(..., description="Username to remove from wishlist")):
